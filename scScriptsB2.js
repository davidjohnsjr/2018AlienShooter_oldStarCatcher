...
window.onload = function() {
    ...
    player1Img.src="images/spaceship3.png";
...
// our stars are created using a single array with a class of information
    ...
    function playerUpdate() {
        ...
        if (p2y>h-40) {p2y=0;}
        if (p2y<0) {p2y=h-40;}
        if (p2x>w-40 && p2x<w) {p2x=0;}
        if (p2x<0) {p2x=w-40;}
        if (p1y>h-40) {p1y=0;}
        if (p1y<0) {p1y=h-40;}
        if (p1x>w-40 && p1x<w) {p1x=0;}
        if (p1x<0) {p1x=w-40;}
    ...
    function laserBeam() {
        ...
        for (var i = 0; i < laserCount; i++) {
            ...
            if (laserArray[i]._visible){
                ...
                if (laserArray[i]._x>w || laserArray[i]._x<0) {var temp=true; var j=i; laserArray[i]._xSpeed = 0; laserArray[i]._x = w+50; laserArray[i]._visible=false;}
                if (laserArray[i]._y>h || laserArray[i]._y<0) {var temp2=true;var m=i; laserArray[i]._ySpeed = 0; laserArray[i]._y = h+50; laserArray[i]._visible=false;}
                for (var k = 0; k < starCount; k++){
                    if (Math.abs(laserArray[i]._x-starArray[k]._x)<40 & Math.abs(laserArray[i]._y-starArray[k]._y)<20) {
                        ...
                        else if (laserArray[i]._wp==2){scoring(i,k,2);}
                    }

                } //endFor
            } //end if visible
        }//endFor
        if (temp) {laserArray.splice(j,1); laserCount = laserArray.length;}
        if (temp2) {laserArray.splice(m,1); laserCount = laserArray.length;}
  ...
  //  scoring functions to place and score stars
    ...
    function gameOver(wp) {
        if (p1Score>hiS) {
            hiS = p1Score;
            $("#hs").text(hiS);
            ctx.fillText("New high score",w/4,h/6);
        }
        if (p2Score>hiS) {
            hiS = p2Score;
            $("#hs").text(hiS);
